...
class WarshallFloyd():
  def __init__(self, N):
    self.N = N
    self.d = [[float("inf") for i in range(N)] for j in range(N)]
    self.prev = [[i for j in range(N)] for i in range(N)]
    for i in range(N):
      self.prev[i][i] = i
...
for i in range(N):
  for j in range(i+1,N):
    now = j
    while now != i:
      prev = graph.prev[i][now]
      used.add((now,prev))
      used.add((prev,now))
      now = prev

print(len(dic-used)//2)